  ...
  async run () {
    ...
    try{
      const user1 = await Database.raw(`
        INSERT INTO freshwear.users (username, email, password, f_name, l_name)
    ...
    try{
      const user3 = await Database.raw(`
        INSERT INTO freshwear.users (username, email, password, f_name, l_name)
        Values('customer3', 'customer3@gmail.com', '${password}', 'Bob', 'Smith')
        `);
    ...
    try{
      const user4 = await Database.raw(`
        INSERT INTO freshwear.users (username, email, password, f_name, l_name)
        Values('service1', 'service1@gmail.com', '${password}', 'Ron', 'Smith')
        `);
    ...
    try{
      const user5 = await Database.raw(`
        INSERT INTO freshwear.users (username, email, password, f_name, l_name)
        Values('admin1', 'admin1@gmail.com', '${password}', 'Rick', 'Smith')
        `);
