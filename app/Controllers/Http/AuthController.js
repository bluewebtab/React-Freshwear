...
class AuthController {
    ...
    async storeUser({response, request, view, session}){
        ...
        if(request.input('password') == request.input('confirm_password')){
         ...
         if(validation.fails()){
            ...
            session.flash({success: 'Welcome to FreshWear'})
    ...
    async handleLogin({response, request, view, auth, session}){
        ...
        let user = await Database.raw(`
            SELECT * FROM freshwear.users WHERE users.email = ${sanitize.escape(postData.email)} LIMIT 1`);
        user = user[0][0];
        if(user){
            //verify the password
            ...
            if(passwordVerified){
                ...
                session.withErrors([{field: 'password', message: 'Incorrect password'}
